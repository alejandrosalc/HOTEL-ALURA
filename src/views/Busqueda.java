	    ...
	    private void actualizarHuespedes() {
	        Optional.ofNullable(modeloH.getValueAt(tbHuespedes.getSelectedRow(), tbHuespedes.getSelectedColumn()))
	                .ifPresentOrElse(filaHuespedes -> {
	                    ...
	                    Integer id_huesped = Integer.valueOf(modeloH.getValueAt(tbHuespedes.getSelectedRow(), 0).toString()); // Obtener el ID del huésped

	                    this.huespedesControl.actualizar(nombre, apellido, fechaNacimiento, nacionalidad, telefono, id_reserva, id_huesped); // Pasar el ID del huésped
